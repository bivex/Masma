...
from masma.application.dto import (
...
from masma.domain.events import (
    ParsingJobCompleted,
    ParsingJobStarted,
    SourceUnitParsed,
    SourceUnitParsingFailed,
)
from masma.domain.model import ParseOutcome, ParseStatus, ParsingJob, SourceUnit
from masma.domain.ports import (
    Clock,
...
def _map_source_outcome(outcome: ParseOutcome) -> SourceParseReportDTO:
    return SourceParseReportDTO(
        source_location=outcome.source_location,
        parser_version=outcome.parser_version.value,
