...
from masma.domain.ports import ControlFlowExtractor, NassiDiagramRenderer, SourceRepository
...
class NassiDiagramDocumentDTO:
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "procedure_names": list(self.procedure_names),
        }
...
class NassiDiagramService:
    ...
    def build_directory_diagrams(self, command: BuildNassiDirectoryCommand) -> NassiDiagramBundleDTO:
        source_units = tuple(self.source_repository.list_source_units(command.root_path))
    ...
    def _build_document(self, source_unit) -> NassiDiagramDocumentDTO:
        ...
        return NassiDiagramDocumentDTO(
            ...
            procedure_count=len(diagram.functions),
            procedure_names=tuple(function.qualified_name for function in diagram.functions),
