...
# extrn / externFP / externNP / EXTERN / EXTERNDEF — external symbol references
EXTERN_RE = re.compile(
    rf"^(?:extern(?:def|FP|NP)?|extrn)\s+(?P<name>{_NAME})(?P<tail>.*)$",
    re.IGNORECASE,
)
# EXTERN/EXTERNDEF with comma-separated name:type list
EXTERN_NAMES_RE = re.compile(
    rf"^(?:extern(?:def)?)\s+(?P<names>.+)$",
    re.IGNORECASE,
)
