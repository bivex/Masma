...
class MasmaError(Exception):
    """Base type for all system errors."""


class BusinessRuleViolation(MasmaError):
...
class InputValidationError(MasmaError):
    """Raised for invalid user input at the system boundary."""


class SourceAccessError(MasmaError):
    """Raised when the system cannot access or decode a source file."""


class GeneratedParserNotAvailableError(MasmaError):
    """Raised when required parser support artifacts are unavailable."""
