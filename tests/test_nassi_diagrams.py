...
""".strip(),
    ...
    diagram = extractor.extract(source)
    # proc_a, proc_b, plus the INIT macro body itself
    assert len(diagram.functions) == 3
    proc_fns = [fn for fn in diagram.functions if fn.kind == "proc"]
    assert len(proc_fns) == 2
    for fn in proc_fns:
        assert isinstance(fn.steps[0], MacroCallFlowStep)
        assert fn.steps[0].target == "INIT"
    macro_fns = [fn for fn in diagram.functions if fn.kind == "macro"]
    assert len(macro_fns) == 1
    assert macro_fns[0].name == "INIT"
